...
RSpec.describe SellOrder, type: :model do
  ...
  describe "scopes" do
    ...
    context "with current_open_sales" do
      include_context "with sell_orders for scopes"
      ...
      it "retrieves the corresponding sell_orders" do
        ...
        travel_to specific_date do
          expect(described_class.current_open_sales.count).to eq(9)
        end
      end
      ...
      it "retrieves the corresponding sell_orders status" do
        ...
        travel_to specific_date do
          expect(described_class.current_open_sales.pluck(:status).uniq).to contain_exactly("opened", "packed", "invoicing")
        end
      end
      ...
      it "retrieves the corresponding sell_orders creation day" do
        ...
        travel_to specific_date do
          expect(described_class.current_open_sales.pluck(:created_at).map(&:day).uniq).to eq([ saturday.day ])
        end
      end
    end
  end
  ...
  describe "#before_destroy callback" do
    ...
    context "when the sell order does have processing order and opened order associations" do
      let(:sell_order_with_orders) do
        create(:sell_order, :with_allocation, :with_orders, trait_amount: 2)
      end

      before { sell_order_with_orders.orders.first.update(status: :processing) }
